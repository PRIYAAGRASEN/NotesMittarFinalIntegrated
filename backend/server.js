...
const blockchain = require('./fabric/Doc_function.js');
...
app.get('/api/my-resources', async (req, res) => {
    ...
    try {
        ...
        const enriched = resources.map(doc => {
            const relevanceScore = Number.isFinite(doc.relevanceScore) ? Math.round(doc.relevanceScore) : null;
            ...
            return {
                ...
                topicCoverage: Array.isArray(doc.topicCoverage) ? doc.topicCoverage : [],
                coverageAnalysis: doc.coverageAnalysis || null,
...
// ✅ Robust Gemini relevance API route
// AI Relevance API with robust Gemini JSON parsing
